...
async function runDemo() {
  try {
    ...
    // --- 4. EXERCICI: SERVICE LAYER CRUD ---
    console.log('\n🏗️  PROJECT SERVICE DEMO:');

    const newProject = await ProjectService.create({
      title: "API Rest Mongoose",
      description: "Seminari de Mongoose i Express",
      organization: initechId as any
    });

    const allProjects = await ProjectService.listAll();
    console.log('Lista de todos los proyectos:', allProjects); 

    const projectDetails = await ProjectService.getById(newProject._id as string);
    console.log('Detalles con Populate:', projectDetails); 
    
    await ProjectService.update(newProject._id as string, { title: "API Rest Mongoose (v2)" });
    
    console.log(`✅ Project Service operations completed`);
    console.log(`Project with Populate:`, projectDetails);
